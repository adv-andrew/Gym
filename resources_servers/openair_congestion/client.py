...
congestion-relief heuristic in place of an LLM policy: each turn it reads the
rendered telemetry, picks the most congested cell, and lifts UL power on it
(`set_ul_power_control`). Per-step tool calls, guardrail verdicts, and
rewards are printed, then the episode return.

If a gym is running (`ng_run` with this server's config), the demo connects
...
# render.to_user_text emits one "- Cell N: ... p99=NN%; ... M SLA violation(s)"
# line per cell; the heuristic reads congestion from those lines, exactly like
# the LLM policy it stands in for.
_CELL_LINE = re.compile(r"- Cell (\d+): .*?p99=(\d+)%.*?(\d+) SLA violation")
...
def choose_action(observation: str, step_idx: int) -> dict[str, Any]:
    """Scripted relief: lift UL power on the most congested rendered cell.

    The p0_dbm sweep keeps consecutive calls distinct, so the guardrail's
    identical-action rate limit never fires.
    """
    cells = [(int(sla), int(p99), int(cell_id)) for cell_id, p99, sla in _CELL_LINE.findall(observation or "")]
    cell_id = max(cells)[2] if cells else 0
    return {
        "name": "set_ul_power_control",
        "arguments": {"cell_id": cell_id, "p0_dbm": -90 + (step_idx % 8), "alpha": 0.8},
    }
...
async def drive_episode(post: PostFn) -> float:
    ...
    while not (terminated or truncated):
        ...
        print(
            f"step {step_idx:2d}: {action['name']}({json.dumps(action['arguments'])}) reward={reward:+.4f} {verdict}"
        )
