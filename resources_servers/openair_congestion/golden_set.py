...
from openair_congestion import render  # noqa: E402
    ...
    for regime in regimes:
        for seed in seeds:
            for prefix_len in decision_steps:
                _episode_id, state = _reconstruct(backend, regime, seed, prefix_len)
...
def score_policy_against_golden(rows: list[GoldenRow], policy: PolicyFn, *, opportunities_only: bool = True) -> dict:
    ...
    for row in rows:
        if opportunities_only and row.margin <= _OPPORTUNITY_MARGIN:
            continue
        _episode_id, state = _reconstruct(backend, row.regime, row.seed, row.prefix_len)
        chosen = policy(render.to_user_text(state))
        value = _value_of(
            backend, row.regime, row.seed, row.prefix_len, ToolCall(name=chosen["name"], arguments=chosen["arguments"])
        )
        scored_rows += 1
